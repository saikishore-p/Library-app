...
export const ChangeQuantityOfBook: React.FC<{ book: BookModel, deleteBook: any }> = (props, key) => {
    ...
    async function increaseQuantity() {
        ...
        // We are calling API but we are not getting any state to recall all the books
        // If we are getting successful response from API, it means we successfully added quantity to database
        // However, it won't reflect automatically because the API is not returning anything. Also we don't want useEffect to recall all books eachtime
        // So, we are physically changing the quantity on the client side by assuming that api made changes in DB and add quantity here for user experience
